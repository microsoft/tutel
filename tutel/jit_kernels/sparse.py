...
def create_forward(samples, global_experts, capacity, aligned_dim, param_dtype):
  return JitKernel.generate_kernel({'capacity': capacity, 'samples': samples, 'hidden': aligned_dim, 'dtype': get_kernel_dtype(param_dtype), 'IS_FLOAT': 1 if param_dtype == torch.float32 else 0}, '''
