...
def create_forward(samples, global_experts, capacity, aligned_dim, param_dtype):
  return JitCompiler.generate_kernel({'capacity': capacity, 'samples': samples, 'hidden': aligned_dim, 'dtype': get_kernel_dtype(param_dtype), 'IS_FLOAT': 1 if param_dtype == torch.float32 else 0}, 0, '''
...
def create_backward_data(samples, global_experts, capacity, aligned_dim, param_dtype):
  return JitCompiler.generate_kernel({'capacity': capacity, 'samples': samples, 'hidden': aligned_dim, 'dtype': get_kernel_dtype(param_dtype), 'IS_FLOAT': 1 if param_dtype == torch.float32 else 0}, 1, '''
    #define capacity (@capacity@)
    #define samples (@samples@)
    #define hidden (@hidden@)
    #define __dtype @dtype@
...
def create_backward_gate(samples, global_experts, capacity, aligned_dim, param_dtype):
  return JitCompiler.generate_kernel({'capacity': capacity, 'samples': samples, 'hidden': aligned_dim, 'dtype': get_kernel_dtype(param_dtype), 'IS_FLOAT': 1 if param_dtype == torch.float32 else 0}, 2, '''
    #define capacity (@capacity@)
    #define samples (@samples@)
    #define hidden (@hidden@)
    #define __dtype @dtype@
