...
def test(model, device, test_loader):
    ...
    penv.dist_print('\nTest set: Average loss: {:.4f}, Validate Accuracy: {}/{} ({:.2f}%)\n'.format(
...
def main():
    ...
    if int(torch.os.environ.get('LOCAL_RANK', 0)) == 0:
        dataset1 = Net.DATASET_TARGET('../data', train=True, download=True,
                           transform=transform)
        dataset2 = Net.DATASET_TARGET('../data', train=False,
                           transform=transform)
        torch.distributed.barrier()
    else:
        torch.distributed.barrier()
        dataset1 = Net.DATASET_TARGET('../data', train=True, download=False,
                           transform=transform)
        dataset2 = Net.DATASET_TARGET('../data', train=False,
                           transform=transform)
    torch.distributed.barrier()
