...
class MOELayer(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if parallel_type.startswith('hybrid:'):
            hybrid_param = int(parallel_type[parallel_type.index(':') + 1:])
            if hybrid_param == 0:
                self.force_data_parallel = True
            else:
                raise Exception('Unimplemented parallel for hybrid_param != 0')
        ...
        self.ffn_zero_group = C.create_groups_from_world(group_count=self.num_global_experts).model_group if self.sharded_count > 1 else None
