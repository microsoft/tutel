    ...
    def update(self, indices_, locations_, gates_, capacity=None):
        ...
        if sample_size != self.expected_sample_size or capacity != self.capacity:
            ...
            if tuple((sample_size, capacity)) not in self.kernel_pool:
                self.func_fwd = jit_kernel.create_forward(sample_size, self.num_global_experts, self.capacity, self.aligned_dim, self.dtype, is_cuda=indices_[0].is_cuda)
                self.func_bwd_data = jit_kernel.create_backward_data(sample_size, self.num_global_experts, self.capacity, self.aligned_dim, self.dtype, is_cuda=indices_[0].is_cuda)
                self.func_bwd_gate = jit_kernel.create_backward_gate(sample_size, self.num_global_experts, self.capacity, self.aligned_dim, self.dtype, is_cuda=indices_[0].is_cuda)
